...
| **L1 — module library flow** (round 4) | game-amora | `C:/Users/taren/Desktop/Amora/wt-r4-library` / `wt/r4-library` | killed by the session limit after opening its PR; resumed for the report | brief + amendments + addenda R36/R37 (sign-in gate on every members-only page) | **PR #23 open** at `3aa4c8d`, MERGEABLE, verify SUCCESS ×2; merge on report | 3aa4c8d | — |
| **L2 — How Power Is Held** (round 4) | game-amora | `C:/Users/taren/Desktop/Amora/wt-r4-power` / `wt/r4-power` | security review delivered clean; killed twice by the session limit; resumed for the formal report | 14-point map rebuild, shape/decides-by/domains (+`other`), vision blocks, currency (CHF/CRC + user display + fx job), `org.declare` + `represents_circle` ADR, **the R39 dialer fix**, migration **0083** | **MERGED per R38 → main `3d3db34`** (PR #24, verify ×2 on `fd967fe`, tightening `{method, gloss}` applied); formal report + live probe pending | 3d3db34 | — |
| **L5b — calendar community** (round 4) | game-amora | `C:/Users/taren/Desktop/Amora/wt-r4-calendar2` / `wt/r4-calendar-community` | reported VERIFIED 2026-08-21 (tip `5b9b7b5`, PR #22, verify ×2, 90 files 1410 tests 0 skipped 528 s, security review no HIGH/MEDIUM ≥0.7) | waitlist (promotion inside the freeing transaction), slots, meet-me, layer UI + public-draft approval, who-is-here (counts below tier, keys audited), print, weekly zero-token brief (village-time job, opt-out, agent-inbox delivery), `setOpportunitiesProvider` seam; migration **0088** | **MERGED → main `252c874`**; DONE pending CI on merge + live probe (background) | 252c874 | — |
