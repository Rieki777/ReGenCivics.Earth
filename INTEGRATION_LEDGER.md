...
| HS — hub PII exposure fix (security) | ReGenCivics.Earth | `regen-pii-fix` / `wt/pii-fix` | dispatched 2026-08-15 ~14:40 EDT | `playerProfiles.list` / `getByHandle` publicProcedures → public projection for anonymous callers (no email/wallet/coords), full record for owner; keys-allowlist regression test; survey of other whole-row public procedures | dispatched; PR flow | hub origin/main | at dispatch |
