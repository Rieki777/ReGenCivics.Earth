...
| K1 — assistant cost: router + prefetch + caching (round 3) | game-amora | `wt-cost` / `wt/assistant-cost` | dispatched 2026-08-15 ~13:10 EDT (**1st handle** of K pair) | `assistant.ts` (prefetch option, no-tools line, cache_control if prefix ≥4096), `assistantRouter.ts` + `assistantTemplates.ts` (new), `assistantUsage.ts` (`path`), migration **0081** (`assistant_usage.path`), organize route body, `assistant.test.ts` | dispatched; PR flow; ten-question before/after (spend cap $0.15) | ≥15f378d | at dispatch |
| K2 — synthesis → Batch API (round 3) | game-amora | `wt-batch` / `wt/synthesis-batch` | dispatched ~13:10 EDT (**2nd handle**) | `synthesisBatch.ts` (new), migration **0082** (`synthesis_batches` + items), synthesis-path hunks in index.ts + `synthesis-batch-poll` job, one gameVariables entry, new e2e file, runbook lines | dispatched; PR flow; sync admin path stays synchronous | ≥15f378d | at dispatch |
...
| P — $ReGen builders' pool (round 3) | hub (`regen-pool` / `wt/pool`) + game-amora (`wt-pool` / `wt/module-pool`) | two worktrees | dispatched ~11:30 EDT; **died on network outage ~12:05, resumed with commit-first + R23(3)** | hub: design ADR, cycle statement job, statement tables + export, public counts page, `pool.regen_per_cycle`; game-amora: `shared/modulePool.ts` derived `poolStatus`, `builtBy.payout`, payload projection, federation byte-identical proof | dispatched; game-amora branch lands before D; PR flow | hub 0b705f8 / ga ≥9632450 | at dispatch |
