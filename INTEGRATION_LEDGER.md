...
| C — module library platform | game-amora | `wt-library` / `wt/module-library` | reported + landed 2026-08-14 | `shared/modules.ts`, `server/lib/modules.ts`, `secrets.ts`, `integrations.ts` + `memberDrivers.ts` + `moduleDocProvenance.ts` (new), migration **0079**, catalog + Integrations UI, `NotifyPrefsPanel.tsx` (C2 member-facing farewell), index.ts zones per its anchor table | **LANDED on main `d14b160`** (fast-forward; coordinator-verified: cherry = 1 commit, 29-file surface, zero boundary violations, NotifyPrefsPanel verified from content as C2 item 6). 11 gates green, 1062 passed / 0 skipped vs 0 baseline, 10.3 min. CI + live probe in flight | d14b160 | 2026-08-14 (final report) |
...
| 2 | Lane C phase C1 (catalog, tier metadata, 503 lapse, dynamic secret slots, registry-driven cards, tier stamped at enable, **+ integration_health + correlation-id driver wrapper + liveness-window field** — promoted, §8-R6) | — | **LANDED** main `d14b160` (C1 AND C2 in one commit); CI + live read in flight |
...
| 13 | Wire `sectionCitation` provenance rendering: one-line change in `knowledge.ts` (was Lane A's file, lane closed), written out in `server/lib/moduleDocProvenance.ts`'s header + ARCHITECTURE checklist | own small dispatch, with item 11 | queued |
