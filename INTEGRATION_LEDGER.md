...
| 22 | **CI flake mechanism found by F2**: `GET /api/admin/audit` returns only the last 200 events; `loop.e2e.test.ts` S9 (~1008) sits late in a 64-test order-dependent file, so its mint row can fall outside the window; unchanged re-runs pass. Routed to F4 (server zone): `?limit=` capped, or S9 filters by its own event ref; default route behaviour unchanged | F4 | added to F4's scope 2026-08-15 |
