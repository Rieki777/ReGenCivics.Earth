...
| R — live acceptance run + test admin (round 3) | live village | none (read-only checkout `wt-liveqa` for code reading) | dispatched 2026-08-15 ~12:20 EDT | creates `integration-qa` (R23), mints short-lived JWTs from the server secret via Railway (never persisted), preflights `village_record` + key presence, runs ~10 organize questions (~$0.10), reads `assistant_usage`/`rate_hits` read-only, screenshots the citation line | dispatched | live build at run time | at dispatch |
