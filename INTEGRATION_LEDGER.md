...
| 36 | `/quests` Propose CTA wholly under the tab bar at first paint on 375×812 (V2); `/feed` first-card actions at 390×844 — R26 below-fold-reachable, cheap layout move | F1 resumed (`wt/quests-cta-375`) | in progress |
| 37 | Store catalog 360 px overflow REGRESSED 0 → 10 px after D's #10 (cards fixed at 298); + queue 27 admin sign-in a11y | F3 resumed (`wt/store-360-and-signin`) | in progress |
