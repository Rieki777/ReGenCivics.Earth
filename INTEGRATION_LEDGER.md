...
| L — full live test (round 2) | live deployment (build `5f3cf0b`) | audit-only; local build in `wt-liveqa` for admin/write surfaces | dispatched 2026-08-15 01:15 EDT (background agent, opus; **first handle of the L/V dispatch pair**) | AUDIT ONLY this phase: functional, desktop 1280 + tablet 768, accessibility, content/data-integrity; store surfaces on local build; federated-doc grep; 401/404/503 semantics | dispatched | 5f3cf0b (target) | at dispatch |
| V — mobile-first QA (round 2, R14) | live deployment (build `5f3cf0b`) | audit-only; local build in `wt-liveqa-v` for admin surfaces | dispatched 2026-08-15 01:15 EDT (background agent, opus; **second handle of the pair**) | AUDIT ONLY: Playwright WebKit iPhone 14 390×844 DPR3 + 375 + 360; journeys, touch targets (measured), Safari hazards, store one-handed, mobile perf; screenshots to scratchpad/lane-v/ | dispatched | 5f3cf0b (target) | at dispatch |
...
| 18 | **Round-2 QA fix lane(s)**: findings from L+V, deduped and triaged by the coordinator, HIGH first, routed so no two lanes share a file | 16, 17 reports | queued |
