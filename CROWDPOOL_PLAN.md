...
| 2026-09-24 | **Village-os creates the contribution tokens for every crowdpooling contribution.** Money that comes from the fund: the fund receives those tokens. Equipment and time from individuals: those individuals receive the tokens directly. The hub records; the village issues. (Rye's words.) |
| 2026-09-24 | **The fund owns a share of every land project.** "That's the whole point of the alliance: the fund is representing a stake of all of them, so we're all interconnected and owning a part of each other." (Rye's words.) This replaces the 2026-09-24 suggestion that the fund's share of a village-only token stay a hub record by default: the fund holds a real balance in each project. |
| 2026-09-24 | **How village-os fits these rulings** (its economics session, measured on its main): (a) redemption already burns the token in the village and something real happens elsewhere, so a project token can redeem to Base: burn in the village, mint on Hypha, with no Hypha token ever on the village ledger; the one change is a destination kind "tokens on Base" beside cash. (b) The fund holds a named outside-party account in each village, never a member account; because a holder can redeem, **the fund's stake is a claim on each village's money, bounded by the village-wide redemption cap per moon**, and villages should see it in those words. (c) Contributors with no village account: their tokens are held in an escrow account against their hub identity and released when they join; never minted to an account with no person behind it. Open with Rye: verify Base redemptions on chain or have a steward attest (village-os recommends verify); whether one village can host more than one project. |
| 2026-09-25 | **Every village names its closing policy before it launches.** Rye, via the village-os economics session: "before you launch the village you have to articulate what it means to close a village", with the default that "everyone holding contribution tokens gets an equal share of the treasury and assets"; a village may name another. **At dissolution the fund is one holder among the others under that village's policy, with no priority. The hub never promises the fund any wind-down treatment**, because the village decides it before the fund holds anything. Open with Rye (tracked by village-os): whether "holding" means closing-day balance or lifetime contribution; if lifetime, the hub's contribution record becomes the durable record of who contributed what. |
