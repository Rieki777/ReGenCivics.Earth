...
export async function dispatchExecution(proposalId: number): Promise<{ status: string; detail?: string }> {
  ...
  // A demonstration proposal never executes. It cannot reach this path through
  // the normal pipeline (launchOnHypha refuses it), but this guards any direct
  // or seeded call so no example ever touches real game state.
  if (proposal.isExample) return { status: "skipped", detail: "Demonstration proposal: execution is a no-op" };
...
export async function advanceLaunchWindows(): Promise<{ merged: number; waiting: number }> {
  ...
  const [rows] = await database.execute(
    sql`SELECT e.id, e.proposalId, e.detail FROM governance_executions e
        JOIN proposals p ON p.id = e.proposalId
        WHERE e.kind = 'feature' AND e.status = 'shipping' AND p.isExample = 0 LIMIT 25`
  );
  let merged = 0;
  let waiting = 0;
