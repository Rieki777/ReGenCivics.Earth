/**
 * The deterministic Galley remix engine (Galley spec section 6d).
 *
 * Rules-based, no LLM. Given the ingredients a crew logged (their market haul
 * plus what is aboard) and a chosen track, it scores every GALLEY_CARD by how
 * much of the haul it can use, then renders the top matches as named dishes built
 * only from what the crew actually has. When the haul is thin it suggests the two
 * or three items to grab next from the seasonal staples.
 *
 * Deterministic-first (STEERING): this must work fully without the Ship's Cook.
 * Pure and unit-tested (server/galley-remix.test.ts). No randomness, so the same
 * haul always remixes the same way.
 */
import {
  GALLEY_CARDS,
  INGREDIENT_ALIASES,
...
/** Pick one valid remix at random-free "roll" (index into the ranked dishes). */
