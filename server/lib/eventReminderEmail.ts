...
export function buildAutoReminderHtml(input: ReminderEmailInput): string {
  const dateStr = input.startTime.toLocaleDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  ...
  const joinUrl = input.joinUrl || `${APP_BASE_URL}/schedule`;
  ...
  return `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
          ...
          <div style="padding:30px 24px;background:#fff;border:1px solid #e0e0e0;border-top:none;">
            ...
            ${bodyHtml}
            <a href="${joinUrl}" style="display:inline-block;background:#7c3aed;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:15px;margin:0 8px 8px 0;">Join on Riverside</a>
