...
async function cascadeCrowdpoolPassed(bridgeRow: any, txHash: string | undefined): Promise<void> {
  ...
  // Notices ride the notification spine (bell, push, email by prefs) to the
  // contributor and every project steward, deduped cp:chain:{id}:u{uid}.
  // notifyChainConfirmed never throws.
  if (campaign) {
    const { notifyChainConfirmed } = await import("../campaign-notify");
    await notifyChainConfirmed({
