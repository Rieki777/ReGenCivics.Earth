...
const ITEMS: Item[] = [
  ...
  { slug: "paddle-ball-set", name: "Paddle ball set", category: "adventure", lore: "A little friendly rivalry by the water.", description: "Wooden paddles and balls for the beach, a meadow, or a campsite.", quantity: 1, storagePlace: "Adventure bin, rear bay", activityTags: ["lake day"] },
  ...
  { slug: "gravity-drinking-water-filter", name: "Gravity drinking-water filter", category: "water", lore: "Slow water is the sweetest.", description: "A countertop gravity filter for drinking water on top of her whole-RV filtration.", quantity: 1, storagePlace: "Galley counter", activityTags: ["hosting dinner", "rainy day"] },
  ...
  try {
    for (let i = 0; i < ITEMS.length; i++) {
      ...
      if (Array.isArray(existing) && existing.length > 0) {
        await conn.execute(
          "UPDATE ship_inventory_items SET name = ?, category = ?, lore = ?, description = ?, quantity = ?, storagePlace = ?, activityTags = CAST(? AS JSON), isGearChecked = ?, sortOrder = ? WHERE slug = ?",
          [it.name, it.category, it.lore, it.description, it.quantity, it.storagePlace, tags, gear, i + 1, it.slug],
        );
        updated++;
      } else {
        await conn.execute(
          `INSERT INTO ship_inventory_items (name, slug, category, lore, description, quantity, storagePlace, activityTags, isVisible, isGearChecked, sortOrder, createdAt, updatedAt)
           VALUES (?, ?, ?, ?, ?, ?, ?, CAST(? AS JSON), 1, ?, ?, NOW(), NOW())`,
          [it.name, it.slug, it.category, it.lore, it.description, it.quantity, it.storagePlace, tags, gear, i + 1],
        );
        inserted++;
      }
    }
