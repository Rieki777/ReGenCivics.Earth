...
## ADR-49: Managed-tier listings run on a platform-held, env-only credential — a scoped exception to "never stored in ReGen Civics systems"

- Date: 2026-08-14. Status: **Accepted** (Rye, 2026-08-14, in the coordinator session). Scopes (does not repeal) CUSTOM_GAMES_MASTER_PLAN locked decision 1.
- Context: the module library (game-amora) sells three listing tiers cut by who bills and who supports. The Managed tier is "one bill, from us; one number to call, ours": CORE bills the village, the vendor invoices CORE, and the village has no vendor account at all. The tier's mechanical definition is the credential plane — there is nothing for the village to hold. CUSTOM_GAMES_MASTER_PLAN locked decision 1 reads "API keys post-acceptance only, entered by the client into their own instance. Never at intake, never stored in ReGen Civics systems," enforced in `shared/customGameBlueprint.ts`. That decision governs the CLIENT's keys and never contemplated a platform-owned key. game-amora already ships the exact posture Managed needs: `PLATFORM_ASSISTANT_KEY` is read from env at call time, deliberately kept out of `SECRET_KEYS`, never returned by the secrets route (not even masked), and disclosed in-product by the `assistant-own-key` launch requirement ("running the guide on a key the platform lends it... when it rotates, your guide stops answering"). ADR-46 is the hub-side precedent for one platform-held vendor account serving every fork; its recorded trade-off — fork secrets plaintext in the hub DB with a revisit flag — must be answered here, not inherited.
- Decision: a Managed listing's vendor credential is platform-OWNED and lives ONLY in the fork deployment's environment configuration, read at call time. Never in any database (hub or fork), never in `SECRET_KEYS`, never rendered to any admin surface, not even masked, because it is not the village's secret to see. Scope guards: (1) applies only to platform-owned keys for Managed-tier listings — the village's own keys (Included/Connected) remain exactly under locked decision 1, entered by the client into their own instance; (2) Managed is hard-capped at two concurrent listings, the second explicitly a transition slot; (3) every Managed listing ships an in-product disclosure in the `assistant-own-key` pattern, so the village is told inside the product that this capability runs on a lent key; (4) Managed carries a data-return obligation on withdrawal, because the village never had the account; (5) rotation is a Railway env update + redeploy, owned by Rye — instant revocation needing no cooperation from the fork.
- Why: the credential is the entitlement — `setModuleLifecycle` makes no network call, reads no secret, checks no licence, and a fork owns its own source, so a module flag can never be a paywall. The one plane the fork cannot edit is the key the platform holds. Managed is the tier that makes the single-biller model (CORE invoices the village inside `regen-full-service`) mechanically real, and it generalises a posture already shipped and disclosed rather than inventing a new one.
- Trade-offs: the ownership promise ("100% theirs: code, data, keys; no subscription required") stays true of the game itself and must be RE-STATED per Managed listing: the owner's guide gains the sentence saying which capability is rented and stops the day the platform key rotates (the `FORK_RUNBOOK.md` borrowed-Anthropic-key precedent, extended per listing). Answering ADR-46's plaintext flag: this plane stores NOTHING at rest in any database — no hub table, no fork table, no masked read — which is strictly stronger than the fork-relay posture; the hub-side roster of which forks are entitled (the Lane H build) holds entitlement facts only, never key material. Residual risk accepted: anyone with Railway access to a fork's service can read its env, which is already true of `PLATFORM_ASSISTANT_KEY` and of every env-fallback secret; the mitigation is rotation ownership, not storage.
- Code refs (game-amora): `server/lib/assistant.ts` (`resolveKey`, `platformDailyCap`, the `SECRET_KEYS` exclusion comment), `shared/launchRequirements.ts` (`assistant-own-key`), `docs/FORK_RUNBOOK.md:27`. Hub: `shared/customGameBlueprint.ts` (locked decision 1's enforcement), ADR-46 (`server/lib/hypha-bridge/fork-relay.ts`). Program: `docs/integration-program/MODULE_LIBRARY_TIERS_AND_PROCESS_2026-08-14.md`, `INTEGRATION_LEDGER.md` §3b/§8.
