...
- Decision, the cycle: pool cycles key on the Meeus reference new moon (2000-01-06 18:14 UTC) so a hub cycle number names the same lunation as a village gratitude cycle number. This surfaced a real disagreement: `server/lib/lunar.ts` here uses a 2025-01-29 12:36 UTC epoch, `game-amora` `shared/lunar.ts` uses Meeus, they are 310.0096 lunations apart, and their cycle boundaries sit 6.79 hours apart. The fork's file describes itself as a verbatim port of a hub `shared/lunar.ts` that did not exist. It exists now, carrying the Meeus constants and the cycle-number math; `server/lib/lunar.ts` is left byte-identical because gratitude and the existing crons run on it and moving a live clock by 6.79 hours for an unrelated feature is its own change (design doc D7).
...
- Code refs: `docs/MODULE_POOL_DESIGN.md` (full design + decision table), `shared/lunar.ts` (new, Meeus cycle numbers), `shared/modulePool.ts` (share arithmetic), `server/jobs/moduleBuildersPool.ts`, `server/_core/index.ts` (`POST /api/cron/module-pool-statement`), `drizzle/0227_module_builders_pool.sql`, `scripts/module-pool-statement.ts`. Fork side: `game-amora` `shared/modulePool.ts` + `shared/modulePool.test.ts` (`poolStatus`, `builtByAccount`), `shared/modules.ts`, `server/index.ts` (`pool` on `/api/modules` and `/api/admin/modules`).
