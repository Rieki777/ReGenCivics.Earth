...
- **Review fixes (2026-09-24, second pass)**, pinned by `server/campaign-security.test.ts` section 7:
  ...
  - Sign-in links draw on their own hourly email budget (`sendEmail({ budget: 'auth' })`), so a burst of campaign notices or offers can no longer use up the shared 50-an-hour cap and stop people signing in. Fan-out campaign notices (update, cancelled, completed, role filled, role opened up again) email by the daily digest.
- **Money routes, loans and readiness ticks (2026-09-25).** Each new campaign write goes through `server/lib/project-steward.ts`, pinned by `server/money-routes.test.ts`, `server/give-lend.test.ts` and `server/readiness-ticks.test.ts` (the steward check on `addPartnerLink` was planted back once and a test failed):
  - **Adding a route is steward-only; verifying is admin-only.** `campaigns.addPartnerLink`, `removePartnerLink` and `getPartnerLinksForSteward` call `assertCampaignSteward` (a missing campaign answers FORBIDDEN, never NOT_FOUND). `campaigns.reviewPartnerLink` is an `adminProcedure`. A new route is `pending` and invisible: `getPartnerLinks` (public) shows only `verified` rows, plus `example` rows on example campaigns, from an explicit column list that never includes `proofUrl`, `reviewNote`, `addedBy` or `verifiedBy`. Stewards cannot add to example campaigns or remove example routes (admins can). A Steward (loan) route cannot be verified while `crowdpool.rails.loan_routes` is off.
  - **The loan route switch hides, too (2026-09-25 review fix).** It used to gate verification only, so turning it off again after a counsel ruling left verified Steward routes public, linked, counted as lent money and fetched nightly. Every public read now filters through `publicRouteWhere` and `loanRoutesOpen` (`server/db.ts`): `getPartnerLinks`, the Needs tab routes, the progress inputs behind the money line, and `loadVerifiedPartnerLinks`. Stewards and admins still see the route (`getPartnerLinksForSteward`); example routes on example campaigns show either way. Pinned by `server/money-routes.test.ts` (verify with the switch on, turn it off, the route is gone publicly); the filter was planted back and the test failed.
  - **One partner page counts once (2026-09-25 review fix).** The same Ma Earth or Steward page could be added twice (a retry was enough) and verified twice, and the money line then counted it twice, which could falsely read "Money half landed". `addPartnerLink` (inside its row lock) and `create` refuse a page already pending or verified on the campaign (`routePageKey`, `server/lib/partner-links.ts`); `reviewPartnerLink` re-checks the two-per-partner limit and refuses to verify a second copy; `getCampaignProgressInputs` and `getPartnerLinks` count and show one row per page. Pinned by `server/money-routes.test.ts`; each of the three checks was planted back and a test failed.
  - **Returned is steward-only.** `campaigns.markLoanReturned` stamps `returnedAt` on a lend the stewards took on, conditionally, and changes no status or counter.
  - **Readiness ticks are steward-only.** `campaigns.getReadiness` and `setReadinessTick` answer FORBIDDEN to anyone but the project's stewards and admins; keys are checked against `shared/crowdpoolReadiness.ts`, and example and closed campaigns keep no new ticks.
  - **The server sets an offer's value.** `submitContribution` copies a need's per-slot value onto every offer against it and ignores the client's number, bounds every amount (`estimatedValue` and `financialAmount` at most 10,000,000, `quantityPledged` at most 100,000), and refuses `contributionType: 'financial'` while `crowdpool.rails.accept_money` is off. Example campaigns run the same checks and write nothing.
...
**Code**: `server/_core/trpc.ts`, `server/_core/security.ts:timingSafeEqualStr`, `server/lib/project-steward.ts`, `shared/campaignStatus.ts`, `server/lib/partner-links.ts`.
...
- **Campaign widget and project page head (2026-09-25, crowdpool build lane 3).** The `/embed/campaign/:id` widget now reads the two-line bar from `shared/campaignProgress.ts`; the stored title and location are decoded once and then escaped (`escapeHtml(decodeBasicEntities(...))`), so they neither inject markup nor print as `&amp;amp;`, and the link target (`campaignRedirectTarget`) is escaped in its `href`. Two head-injection holes in `server/_core/vite.ts` closed on the way, both reachable now that `/project/:key` puts campaign text into the head: `injectMetaTags` used replacement strings, so a `$2` or `$1` in a description or title was read as a back-reference (a description saying "we need $20,000" put the attribute's closing quote in the middle of `og:description`); every replacement is a function now, pinned by a new case in `server/vite-meta.test.ts` (planted back, it failed). And the JSON-LD block escapes `<` as `\u003c`, so a `</script>` in a project's name or description cannot close the script element early. The crawler content for `/project/:key` (`server/_core/crawler-content.ts getProjectContent`) decodes then escapes every campaign value. The `/campaign/:id` 301 (`server/lib/campaign-redirect.ts`) fires only for a public campaign, so an unpublished project's name never appears in a Location header (pinned by `server/campaign-redirect.test.ts`).

- **Crawler body injection (2026-09-25 review fix).** A third replace in `serveStatic` put the crawler prose in front of `<div id="root">` with a replacement string. `escapeHtml` leaves `$` alone, so `$'` in a campaign description, a need title or a project name pasted the rest of the shell inside the off-screen crawler div: the div closed early and the steward's text sat visibly in `<body>`, outside the app, on every visit to the canonical campaign page (reproduced in a real browser at 375px). `` $` `` copied the whole head into the body and `$&` added a second root. The same line served forum posts. Both crawler injections now go through `injectCrawlerContent` (`server/_core/vite.ts`) with function replacements, pinned by `server/vite-meta.test.ts` (each dollar pattern leaves one root, one head and one script; planted back, four cases failed). No replacement string carrying people's text is left in `vite.ts`.

**Code**: `server/_core/security.ts:sanitizeInput`, `client/src/utils/sanitize.ts`, `shared/htmlText.ts`, `server/routes/embed.ts`, `server/_core/vite.ts:injectMetaTags`, `server/_core/vite.ts:injectCrawlerContent`.
...
- **Share card cache is bounded (2026-09-25 review fix).** `/api/og` is public. It keyed `type=campaign` cards by the raw id string, and `Number()` reads many spellings of one id (`01597`, `1597.0`, `0x63D`, `1597e0`), so each spelling cost a fresh render (about 270 ms of CPU) and a cache entry that was never freed. An id now has to be a plain positive whole number (anything else answers 400 before rendering), the key uses the parsed number, cards that ignore their id share one key, and the map holds at most `OG_CACHE_MAX` (500) cards, oldest out first (`ogCacheKey`, `rememberOgCard` in `server/routes/og.ts`). Pinned by `server/og-campaign.test.ts`; the raw key and the missing cap were each planted back and tests failed.
...
- Magic link returnTo (2026-09-24): `POST /api/auth/email/request` takes an optional `returnTo`, runs it through `normalizeReturnTo()` and stores it in `email_tokens.returnTo` (migration 0254). The emailed URL carries only the token, so the destination cannot be hand-edited. Verify re-runs `normalizeReturnTo()` on the stored value and falls back to `/profile`. `normalizeReturnTo()` now also rejects values over 512 chars, any backslash or control character anywhere, undecodable escapes, and percent-encoded forms of `//`, `\` and control characters through three decoding rounds (`/%2F%2Fevil`, `/%252F%252Fevil`, `/%5Cevil`, `/%09/evil`). The OAuth `state` returnTo gets the same checks. Tests: `shared/oauthReturnTo.test.ts`, `server/auth.email-return.test.ts` (round trip, hostile values, replay).
- Magic link returnTo hardening (2026-09-24, review round): `normalizeReturnTo()` resolves the value the way a browser would (`new URL` against a placeholder origin that must come back unchanged) and returns only the canonical path, query and hash. It refuses server endpoints (`/api`, `/storage`, any case, after dot segments and after one round of decoding), because after sign-in the browser follows the returnTo with the fresh session: a stranger who chose `/api/auth/email/verify?token=<their own>` as the destination of someone's genuine link could swap that person into the stranger's account. It removes secret query parameters (`token`, `preview`, `code`) so unsubscribe and preview tokens never reach `email_tokens` or the OAuth state; AuthDialog runs the same check before sending. An escape that stops decoding after the first round is kept as data (`?q=100%25`). `POST /api/auth/email/request` answers 415 to anything but JSON and 403 when `Sec-Fetch-Site` is `cross-site`, so another site's plain form cannot fire sign-in emails (each of which cancels the person's earlier unused link). `createEmailToken` deletes rows more than a day past expiry, so the table holds about a day of sign-ins. Verify (and the Google and Apple callbacks) waits up to 3 seconds for the anonymous-contribution link before redirecting, so "Your contributions" is there when the page asks.
...
- **Money route hydration (2026-09-25).** Project stewards now supply the partner URLs the nightly job fetches (`campaigns.addPartnerLink`, and `moneyRoutes` on `campaigns.create`), so the fetch is fenced four ways. Every URL passes `validateRouteUrl` (`server/lib/partner-links.ts`) when it is added: https only, no credentials, the default port, and a hostname exactly on the partner's list (`maearth.com`, `www.maearth.com`, `gosteward.com`, `www.gosteward.com`); IP literals, `localhost` and look-alikes such as `maearth.com.evil.test` are refused. The job (`hydrateCampaignPartnerLinks` in `server/routes/batchJobs.ts`) loads only rows a ReGen Civics admin verified (`VERIFIED_PARTNER_LINKS_SQL`), re-checks each URL before fetching, fetches with `redirect: 'manual'`, and follows a 3xx once only when `isAllowedHop` keeps it on the same partner's hosts; anything else counts as failed. Example rows are never fetched. Log lines carry the link id, never the URL. The currency of the numbers is set by the admin at verification, never by the job. Pinned by `server/partner-links.test.ts`, `server/partner-hydration.test.ts` and `server/money-routes.test.ts`; the verified-only filter was planted back once and two tests failed. This had to ship in the same deploy as `addPartnerLink`.
