...
- **Outbound History**: the Outbound admin surface for letters already sent or scheduled (`?tab=outbound&surface=history`). Replaces the Sent label. Stats roll up from `email_logs` (Resend webhooks plus the existing open/click pixel). Not a separate table; rows live in `newsletter_issues`.
