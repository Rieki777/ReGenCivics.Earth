...
**Builders' pool**. The fixed amount of $ReGen ReGen Civics distributes each lunar cycle across the free, third-party modules that villages are actually running. Set by the `pool.regen_per_cycle` game variable. Distinct from `gratitude.pool_per_cycle`, which pays members for recognition received inside one village; the builders' pool pays module builders across the whole network. Say "the builders' pool", never "the module fund" or "the developer pool".

**Pool-eligible**. A module the builders' pool may pay: written by somebody outside the platform (`builtBy` names them), free to the village (`isPaid` is false), still offered (not `withdrawn`), and not `core`. Always DERIVED from the registry entry by `poolStatus`, never stored. A module with a price is not pool-eligible, by the builder's own choice, which is the whole incentive. A module being pool-eligible says nothing about whether any village runs it; the metric is a separate question.

**Cycle statement**. The one document a settled lunar cycle produces: the cycle number and its bounds, the pool, the roster and how each village answered, one line per pool-eligible module with its village count, share and payment state, the balancing terms, and a snapshot hash that makes it reproducible. It is a STATEMENT and never a transaction: the hub writes it, a human reads it and executes the transfers through Hypha. Nothing in this codebase signs anything. Say "cycle statement", never "payout run" or "distribution job" (the job is the thing that WRITES the statement).

**Accrued (a pool share)**. A share earned by a named builder with nowhere to send it, because they hold no ReGen Civics account or have not linked a Base address. It carries into the next cycle for up to three cycles, then lapses to the treasury. Distinct from a **remainder**, which is flooring dust belonging to nobody and is never minted at all. Remainders evaporate, accruals wait.

**Roster (of villages)**. For the pool, `shared/networkRegistry.ts` `NETWORK_GAMES` filtered to `listed: true` and `status: "live"`. The only villages whose module lists ever count. Not a synonym for "the network": the network page lists building villages too, and those count zero.
