...
**Anastasia community presence.** Anastasia's autonomous forum behavior (ADR-21): she comments on new community posts and replies once to direct replies to her comments, in her canon voice, grounded in retrieved passages. Deterministic-first: a 6-hour DB poll (`server/jobs/anastasiaForumJob.ts`) finds the work at zero token cost; only the comment text is a model call. Posts as the `bot:anastasia` user, whose `createdAt` is the no-backfill cutoff. Skips administrative categories (edit-in-place slug list), crisis posts, and posts the model judges do not call for her voice (PASS gate). Every comment carries an AI-presence disclosure. Silenced with `ANASTASIA_FORUM_ENABLED=false`. Her voice and the no-AI-writing rules are the shared `ANASTASIA_VOICE` block, used by both the chat and the forum comments.
