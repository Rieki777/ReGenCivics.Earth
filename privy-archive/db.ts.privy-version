...
export async function getUserByPrivyDid(privyDid: string) {
  const db = await getDb();
  if (!db) return null;
  const rows = await db.select().from(users).where(eq(users.privyDid, privyDid)).limit(1);
 ...
 */
export async function linkOrCreatePrivyUser(privyDid: string, privyProfile: any) {
  const db = await getDb();
  if (!db) return null;

  // Extract email from Privy linked accounts
  const emailAccount = privyProfile.linkedAccounts?.find((a: any) => a.type === "email");
  const googleAccount = privyProfile.linkedAccounts?.find((a: any) => a.type === "google_oauth");
  const appleAccount = privyProfile.linkedAccounts?.find((a: any) => a.type === "apple_oauth");
  const embeddedWallet = privyProfile.linkedAccounts?.find((a: any) => a.walletClientType === "privy");
  const walletAccount = privyProfile.linkedAccounts?.find((a: any) => a.type === "wallet" || a.type === "smart_wallet");
  ...
  if (existingUser) {
    ...
    await db.update(users).set({
      privyDid,
      baseWalletAddress: baseWallet ?? (existingUser as any).baseWalletAddress,
  ...
  const name = googleAccount?.name ?? appleAccount?.name ?? privyProfile.name ?? null;
  ...
  if (newUser) {
    await db.update(users).set({ privyDid, baseWalletAddress: baseWallet } as any).where(eq(users.id, newUser.id));
  }
