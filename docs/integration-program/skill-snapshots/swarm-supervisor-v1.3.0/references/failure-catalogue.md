...
### Three more, from program three

23. **The mutation harness that only asserts `fails > 0`.** A mutation run reported success
    while the baseline it mutated was itself red 3/365 on its own artifact — every mutant
    "failed", including the ones that changed nothing. Prove both directions: the clean tree
    passes before any mutant's failure counts as evidence.

24. **The cold run read as a missed defect.** A lane compared a cold probe (3835-4371ms) against
    a warm suite's numbers (1882-2491ms) and reported "missed 2 of 6". The miss was the
    temperature. A verdict that does not name its run condition is not falsifiable.

25. **The claim measured against a parent the lane was not actually on.** A lane re-derived its
    anchors and its "live stored-XSS" claim against one commit while parented on another; on its
    real parent the defect does not reproduce, because a coercion upstream already handles it.
    Check `git merge-base` for the ACTUAL parent before a defect claim shapes a landing decision.
...
Program three added one more, and it is the largest:

| Error | Cost | Rule it produced |
|---|---|---|
| Asked the founder one question about a governance rule, then reported and waited | Four independent lanes the question did not touch sat undispatched; he had to tell me to keep going | A question gates its lane, never the round; waiting is per-lane |
| Talked the founder into a ruling on an unverified claim, read from a screenshot | The ruling rested on a string that appears in that artifact zero times — it belonged to the shell drawn over it | Verify before it shapes a decision; a screenshot is evidence about the composite |
