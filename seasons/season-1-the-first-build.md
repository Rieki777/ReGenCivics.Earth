# Season 1: The First Build

| Field | Value |
|---|---|
| **Season name** | The First Build |
| **Dates** | Winter 2025-2026 (Dec 2025 to Spring Equinox 2026) |
| **Theme** | Building and Preparing |
| **Seasonal boundary** | Started at Winter Solstice, ends at Spring Equinox |
| **Context** | First season of formalized roles. The site was being built, the game system designed, the incubator shaped. Most roles were open or partially filled. This was the foundation pour. |
| **Art style** | Solarpunk / elven / regenerative future. Hand-painted illustrated style. Bioluminescent accents, living architecture, crystal and mycelium. Every scene full of fruiting plants and comfortable wildlife. |
| **Color palette** | Deep forest greens, warm golds, bioluminescent teals/cyans, sunrise amber, living wood browns, soft white light |
| **Total base budget** | 7,400,000 $ReGen ($74,000) across all 13 roles |
| **Max budget (all bonuses)** | 9,620,000 $ReGen ($96,200) if every role hits both Seed and Harvest |

---

## Compensation Bands

| Band | $ReGen/Season | USD Value | Roles in This Band |
|---|---|---|---|
| 7 | 900,000 | $9,000 | Grand Builder |
| 6 | 800,000 | $8,000 | Game Designer, Security Reviewer |
| 5 | 700,000 | $7,000 | Skills Builder, Season Facilitator |
| 4 | 600,000 | $6,000 | Alliance Weaver, Treasury Steward |
| 3 | 500,000 | $5,000 | Storyteller, Incubator Guide, Tool Curator |
| 2 | 400,000 | $4,000 | Quest Steward, Outreach Writer |
| 1 | 300,000 | $3,000 | Forum Gardener |

**Bonus structure:** Base + up to 30% variable via Seed and Harvest.
- Plant your seeds AND the harvest comes in = +30%
- One without the other = +15%
- Neither = base only

---

## Roles

### 1. Season Facilitator

| Field | Value |
|---|---|
| **Character name** | The Gardener |
| **Tagline** | Keeps the seasons turning |
| **Circle** | Incubation Circle |
| **Band** | 5 |
| **Base** | 700,000 $ReGen ($7,000) |
| **Max w/ bonus** | 910,000 $ReGen ($9,100) |
| **Est. hours/week** | 12 |
| **Assignment** | Filled, seeking 1-2 co-facilitators |
| **Active seasons** | Spring |
| **Card image** | `/images/roles/season-facilitator-card.webp` |
| **Scene image** | `/images/roles/season-facilitator-scene.webp` |

**Deliverables (compensation is tied to these):**
- Facilitate weekly incubation sessions
- Track project milestones and flag blockers
- Write season wrap-up report
- Onboard incoming projects at season start

**Seed:** All scheduled sessions held on time through the season.
**Harvest:** Land projects report feeling supported (Season Festival survey, target: 4+/5 average).

**Powers:**
- Set the incubation session schedule
- Approve resource requests under $500
- Invite guest mentors to sessions
- Pause a project's timeline if they need breathing room

**Rights:**
- Access to all project dashboards and progress data
- Direct line to alliance partners for project support
- First look at incoming land project applications

**Domains:** Season incubation process, project support protocols, session design

**Character description:** Serene person with warm brown skin and silver-streaked hair woven with tiny living flowers. Flowing robe in shifting seasonal colors. Holds a circular holographic seasonal wheel. Living-wood staff with leaves that change color. Subtle pointed ears.

**Scene description:** A circular garden where four seasons exist simultaneously. Cherry blossoms heavy with fruit, wildflowers with bees, autumn apple and persimmon trees, frost-touched evergreens with a fox curled beneath. Crystal sundial. Songbirds on a branch.

---

### 2. Alliance Weaver

| Field | Value |
|---|---|
| **Character name** | The Weaver |
| **Tagline** | Connects what wants to be connected |
| **Circle** | Alliance Circle |
| **Band** | 4 |
| **Base** | 600,000 $ReGen ($6,000) |
| **Max w/ bonus** | 780,000 $ReGen ($7,800) |
| **Est. hours/week** | 10 |
| **Assignment** | Open |
| **Active seasons** | Spring, Summer |
| **Card image** | `/images/roles/alliance-weaver-card.webp` |
| **Scene image** | `/images/roles/alliance-weaver-scene.webp` |

**Deliverables:**
- Maintain active contact with 10+ alliance partners per season
- Report on partnership health monthly
- Coordinate investor communications with Treasury Steward
- Facilitate alliance partner onboarding

**Seed:** New partnership conversations opened (target: 3+ per season).
**Harvest:** At least one partnership that resulted in tangible support for a land project.

**Powers:**
- Initiate partnership conversations on behalf of ReGen Civics
- Draft alliance agreements for community review
- Represent the project at conferences and events
- Recommend alliance tier classifications

**Rights:**
- Access to investor pipeline and CRM data
- Use of ReGen Civics brand assets for outreach
- Budget allocation for relationship-building activities

**Domains:** Alliance partnerships, investor relations, conference representation

**Character description:** Tall, graceful person with deep copper skin and long braids threaded with bioluminescent fibers. Flowing cape with mycelium-network pattern. Threads of golden-green light extend from fingertips. Crystalline pendant pulses with heartbeat glow.

**Scene description:** Living-wood bridge between treehouse communities. Grapevines heavy with fruit on railings. Hummingbird near a flowering vine. Butterflies between treehouses. Fig trees line the valley floor.

---

### 3. Incubator Guide

| Field | Value |
|---|---|
| **Character name** | The Guide |
| **Tagline** | Walks beside new roots |
| **Circle** | Projects Circle |
| **Band** | 3 |
| **Base** | 500,000 $ReGen ($5,000) |
| **Max w/ bonus** | 650,000 $ReGen ($6,500) |
| **Est. hours/week** | 10 |
| **Assignment** | Open, 2 positions |
| **Active seasons** | Spring, Summer |
| **Card image** | `/images/roles/incubator-guide-card.webp` |
| **Scene image** | `/images/roles/incubator-guide-scene.webp` |

**Deliverables:**
- Guide 3-4 land projects per season
- Check in with each project weekly
- Document project progress and lessons learned
- Connect projects with relevant tools from the Tools Library

**Seed:** Weekly check-ins with every guided project completed through the season.
**Harvest:** Guided projects hitting their own self-set milestones (target: 70%+ on track).

**Powers:**
- Approve land project applications for community review
- Assign mentors from the alliance network
- Adjust project milestones based on ground conditions
- Escalate issues to the Season Facilitator

**Rights:**
- Direct access to all land project contacts and data
- Authority to schedule emergency support sessions
- Input on project evaluation criteria

**Domains:** Project intake, milestone tracking, mentor matching, project support

**Character description:** Sturdy person with warm olive skin, kind crinkled eyes, short curly hair with small leaves growing in it. Practical tunic with many pockets. Glowing orb projecting a holographic trail map. Young sapling in a living-soil pouch at hip.

**Scene description:** Forest path with young saplings. Berry bushes with blueberries and raspberries. Living-wood archways with bioluminescent vines bearing glowing fruit. Deer and fawn grazing. Rabbit near trailhead.

---

### 4. Forum Gardener

| Field | Value |
|---|---|
| **Character name** | The Tender |
| **Tagline** | Grows conversations into community |
| **Circle** | Community Circle |
| **Band** | 1 |
| **Base** | 300,000 $ReGen ($3,000) |
| **Max w/ bonus** | 390,000 $ReGen ($3,900) |
| **Est. hours/week** | 6 |
| **Assignment** | Open |
| **Active seasons** | Winter, Spring, Summer, Fall |
| **Card image** | `/images/roles/forum-gardener-card.webp` |
| **Scene image** | `/images/roles/forum-gardener-scene.webp` |

**Deliverables:**
- Post 2-3 seed discussions per week
- Respond to new member introductions within 24 hours
- Review flagged content daily
- Write monthly community health report

**Seed:** Seed discussions posted each week throughout the season.
**Harvest:** New members who came back and posted more than once (community retention).

**Powers:**
- Pin and unpin forum threads
- Move posts between categories
- Issue gentle moderation actions (warnings, thread locks)
- Feature community posts on the homepage

**Rights:**
- Access to moderation tools and flagged content queue
- Ability to create forum categories and tags
- Input on community guidelines updates

**Domains:** Forum moderation, community tone, new member welcome, seed content

**Character description:** Young person with rich dark skin and explosion of natural hair adorned with tiny bioluminescent flowers. Loose linen tunic with living-thread embroidery. Kneeling in a garden tending conversation-plants with a crystalline watering vessel. Luminous moth on shoulder.

**Scene description:** Sheltered courtyard garden. Conversation-plants on trellises of woven light. Passionfruit and kiwi vines. Songbird on trellis. Dragonflies over reflective pool. Cat dozing on moss cushion. Strawberry plants spilling over raised beds.

---

### 5. Game Designer

| Field | Value |
|---|---|
| **Character name** | The Architect |
| **Tagline** | Designs the rules we play by |
| **Circle** | Anchor Circle |
| **Band** | 6 |
| **Base** | 800,000 $ReGen ($8,000) |
| **Max w/ bonus** | 1,040,000 $ReGen ($10,400) |
| **Est. hours/week** | 12 |
| **Assignment** | Partially filled, support needed |
| **Active seasons** | Winter, Spring |
| **Card image** | `/images/roles/game-designer-card.webp` |
| **Scene image** | `/images/roles/game-designer-scene.webp` |

**Deliverables:**
- Maintain the game spec (REGEN_GAMES_SPEC_V1.md)
- Balance contribution scoring each season
- Design 2+ new quests per season
- Document all game mechanic changes and reasoning

**Seed:** New quests or mechanics designed and live on the site (target: 2+).
**Harvest:** Players completing those quests (measured by quest completion count).

**Powers:**
- Propose changes to game variables and scoring formulas
- Design new quest types and progression chains
- Draft seasonal event structures
- Recommend citizenship tier adjustments

**Rights:**
- Access to all game data, player analytics, and scoring systems
- Authority to run game experiments with community consent
- Seat on the seasonal council for game-related decisions

**Domains:** Game mechanics, contribution scoring, quest design, citizenship tiers, seasonal events

**Character description:** Sharp-featured person with pale freckled skin, focused eyes behind crystalline glasses, auburn hair pulled back. Fitted vest with geometric bioluminescent patterns. Seated at floating holographic workstation with game board projections and rule-card holograms.

**Scene description:** Sun-drenched treehouse design studio. Mango and avocado trees visible. Ginger cat sleeping on a beam. Bowl of figs and pomegranates. Bees drifting past. Gecko on a vine.

---

### 6. Treasury Steward

| Field | Value |
|---|---|
| **Character name** | The Keeper |
| **Tagline** | Balances seeds and coins |
| **Circle** | Finance Circle |
| **Band** | 4 |
| **Base** | 600,000 $ReGen ($6,000) |
| **Max w/ bonus** | 780,000 $ReGen ($7,800) |
| **Est. hours/week** | 8 |
| **Assignment** | Partially filled, seeking support |
| **Active seasons** | Winter, Spring, Summer, Fall |
| **Card image** | `/images/roles/treasury-steward-card.webp` |
| **Scene image** | `/images/roles/treasury-steward-scene.webp` |

**Deliverables:**
- Process payments within 48 hours of approval
- Publish monthly treasury reports
- Track all fund inflows and outflows
- Coordinate with Alliance Weaver on investor fund allocations

**Seed:** Monthly reports published on time and visible to the community.
**Harvest:** Community reports zero confusion about where money went (Season Festival survey, target: 4+/5).

**Powers:**
- Process approved payments up to $1,000
- Generate financial reports
- Flag suspicious transactions for community review
- Recommend budget allocations for seasonal planning

**Rights:**
- Full access to treasury accounts and transaction history
- Authority to request supporting documentation for expenses
- Seat on seasonal budget planning sessions

**Domains:** Treasury operations, financial reporting, payment processing, budget tracking

**Character description:** Older person with deep brown skin, silver locs, steady trustworthy gaze. Long flowing robe with golden thread like leaf veins. Floating balance scale of crystallized light: seeds on one side, tokens on the other. Holographic ledger orbits their hand. Round golden-tint spectacles.

**Scene description:** Treasury chamber grown from living crystal and wood. Citrus trees with lemons and oranges. Golden grapes woven through crystal ceiling. Owl on high shelf. Tortoise on floor. Everything transparent and visible.

---

### 7. Storyteller

| Field | Value |
|---|---|
| **Character name** | The Storyteller |
| **Tagline** | Turns what happened into what matters |
| **Circle** | Communications Circle |
| **Band** | 3 |
| **Base** | 500,000 $ReGen ($5,000) |
| **Max w/ bonus** | 650,000 $ReGen ($6,500) |
| **Est. hours/week** | 10 |
| **Assignment** | Open |
| **Active seasons** | Winter, Spring, Summer |
| **Card image** | `/images/roles/storyteller-card.webp` |
| **Scene image** | `/images/roles/storyteller-scene.webp` |

**Deliverables:**
- Publish 2+ blog posts per month
- Maintain social media presence (3+ posts per week)
- Write or edit the monthly newsletter
- Run content through the avoid-ai-writing skill before publishing

**Seed:** Content published on cadence (blog, social, newsletter targets met).
**Harvest:** New community members who say content brought them here (signup source tracking, target: 10+/season).

**Powers:**
- Publish to the ReGen Civics blog and social channels
- Approve content submissions from contributors
- Set the editorial calendar
- Commission content from community writers

**Rights:**
- Access to all content skills and brand assets
- Authority to represent ReGen Civics voice publicly
- Input on all public-facing copy

**Domains:** Blog, social media, newsletter, brand voice, public narrative

**Character description:** Expressive person with warm tawny skin and animated dark eyes, sitting cross-legged on floating moss platform. Writing in a journal of living leaf-pages with light-trailing stylus. Story-threads rise as golden bioluminescence. Camera-like crystal device on strap. Feathers in windswept hair.

**Scene description:** Beneath a massive ancient fig and pear tree. Story-threads drift into branches. Fireflies emerging. Fox among listeners. Butterflies on wildflowers. Basket of fresh fruit. Distant solarpunk spires.

---

### 8. Grand Builder

| Field | Value |
|---|---|
| **Character name** | The Tinkerer |
| **Tagline** | Builds the world one tool at a time |
| **Circle** | Tech Circle |
| **Band** | 7 |
| **Base** | 900,000 $ReGen ($9,000) |
| **Max w/ bonus** | 1,170,000 $ReGen ($11,700) |
| **Est. hours/week** | 15 |
| **Assignment** | Partially filled, builders needed |
| **Active seasons** | Winter, Spring, Summer, Fall |
| **Card image** | `/images/roles/grand-builder-card.webp` |
| **Scene image** | `/images/roles/grand-builder-scene.webp` |

**Deliverables:**
- Review all community PRs weekly
- Maintain CI/CD pipeline and deployment process
- Write execution prompts for major features
- Mentor new code contributors

**Seed:** Features and fixes shipped to production through the season.
**Harvest:** Community contributors who merged their first PR (people you enabled).

**Powers:**
- Merge or reject pull requests
- Set technical architecture decisions
- Approve database migrations
- Grant contributor access to the repo

**Rights:**
- Admin access to GitHub repo and hosting infrastructure
- Authority to set code standards and review criteria
- Budget allocation for infrastructure costs

**Domains:** Codebase architecture, PR review, deployment, technical documentation

**Character description:** Strong-armed person with medium brown skin, rolled-up sleeves, focused creative expression. Living-fiber workshop apron with hybrid tools: crystal, living wood, soft-glowing circuitry. Assembling a glowing module. Floating schematics. Plants growing through everything. Goggles with bioluminescent lenses. Sawdust and pollen in hair.

**Scene description:** Open-air workshop at edge of food forest. Banana palms, breadfruit, tomato vines. Robin on workbench. Bees on squash blossoms. Half-built structures merging wood, crystal, and tech. Golden hour.

---

### 9. Security Reviewer

| Field | Value |
|---|---|
| **Character name** | The Ranger |
| **Tagline** | Keeps our digital commons safe |
| **Circle** | Tech Circle |
| **Band** | 6 |
| **Base** | 800,000 $ReGen ($8,000) |
| **Max w/ bonus** | 1,040,000 $ReGen ($10,400) |
| **Est. hours/week** | 10 |
| **Assignment** | Golden opportunity |
| **Active seasons** | Winter, Spring, Summer, Fall |
| **Card image** | `/images/roles/security-reviewer-card.webp` |
| **Scene image** | `/images/roles/security-reviewer-scene.webp` |

**Deliverables:**
- Review all PRs for security vulnerabilities weekly
- Maintain and improve security scanning workflows
- Document security practices and known risks
- Run quarterly security audits

**Seed:** Security reviews completed for every community PR through the season.
**Harvest:** Zero critical vulnerabilities reaching production.

**Powers:**
- Block any PR on security grounds
- Require security-related code changes before merge
- Run security audits on any part of the codebase
- Recommend security tool adoption

**Rights:**
- Access to all security scanning tools and results
- Authority to set security review requirements
- Input on infrastructure security decisions

**Domains:** Security review, vulnerability scanning, secure development practices

**Character description:** Alert, keen-eyed person with dark skin, short natural hair, quiet confidence. Fitted ranger-style outfit in forest greens/golds with bioluminescent circuitry seams. Crystalline spyglass. Translucent shield of woven light with sacred geometry. Scroll of glowing code-symbols.

**Scene description:** Watchtower grown from a living tree on hilltop. Peach-bearing branches with nesting hawk. Wolf-like companion at tower base. Orchards, food forests, garden plots in valley below. Sunrise behind mountains.

---

### 10. Tool Curator

| Field | Value |
|---|---|
| **Character name** | The Librarian |
| **Tagline** | Organizes what the builders make |
| **Circle** | Community Circle |
| **Band** | 3 |
| **Base** | 500,000 $ReGen ($5,000) |
| **Max w/ bonus** | 650,000 $ReGen ($6,500) |
| **Est. hours/week** | 6 |
| **Assignment** | Open |
| **Active seasons** | Winter, Spring |
| **Card image** | `/images/roles/tool-curator-card.webp` |
| **Scene image** | `/images/roles/tool-curator-scene.webp` |

**Deliverables:**
- Review tool submissions within 72 hours
- Write or improve 5+ tool descriptions per month
- Connect tools to relevant quests and seasons
- Track tool usage metrics and report quarterly

**Seed:** Submissions reviewed and cataloged within the season (target: 72hr turnaround).
**Harvest:** Tools actually getting used by land projects (usage tracking).

**Powers:**
- Approve or reject tool submissions
- Edit tool descriptions and categories
- Feature tools on the homepage and in quests
- Recommend tools to specific land projects

**Rights:**
- Admin access to the Tools Library
- Authority to create and modify tool categories
- Input on tool-related quest design

**Domains:** Tools Library curation, tool submissions, tool-quest integration

**Character description:** Thoughtful person with light brown skin, neat shoulder-length hair, calm expression. Living-fiber cardigan over tunic. Crystalline reading glasses on head. Standing before floating shelves of color-coded glowing tools. Holographic labels. Warm mug floating on a platform of woven light.

**Scene description:** Library-workshop hybrid inside living-crystal structure. Herb pots on windowsills. Dwarf lemon tree bearing ripe fruit. Doves roosting in skylight. Tabby cat between shelves. Climbing vines with berries. Organized, peaceful.

---

### 11. Quest Steward

| Field | Value |
|---|---|
| **Character name** | The Cartographer |
| **Tagline** | Maps the paths players walk |
| **Circle** | Community Circle |
| **Band** | 2 |
| **Base** | 400,000 $ReGen ($4,000) |
| **Max w/ bonus** | 520,000 $ReGen ($5,200) |
| **Est. hours/week** | 8 |
| **Assignment** | Open |
| **Active seasons** | Winter, Spring, Summer |
| **Card image** | `/images/roles/quest-steward-card.webp` |
| **Scene image** | `/images/roles/quest-steward-scene.webp` |

**Deliverables:**
- Design 3+ new quests per season
- Write forum seed posts for each quest
- Track quest completion rates and adjust difficulty
- Collaborate with Game Designer on progression balance

**Seed:** Quests designed, written, and live with forum seed posts (target: 3+).
**Harvest:** Players completing those specific quests (completion count).

**Powers:**
- Draft new quests for community review
- Set quest difficulty and reward amounts
- Write seed comments that model good responses
- Recommend quest ordering in the progression chain

**Rights:**
- Access to the quest builder skill and templates
- Authority to propose quest progression changes
- Input on seasonal quest themes

**Domains:** Quest design, forum seed content, progression chain, quest rewards

**Character description:** Adventurous person with light skin, bright curious eyes, windswept reddish hair. Traveler's vest with pockets of bioluminescent ink. Leaning over holographic map that transforms into living landscape. Compass rose tattoo pulsing with light. Explorer's goggles on head.

**Scene description:** Cartographer's station on cliff overlook. Wild olive and walnut trees frame cliff edge. Eagle circling. Holographic map mirroring landscape below with glowing player-walked paths winding through orchards and food forests. Lizard on warm rock.

---

### 12. Outreach Writer

| Field | Value |
|---|---|
| **Character name** | The Herald |
| **Tagline** | Carries the signal outward |
| **Circle** | Communications Circle |
| **Band** | 2 |
| **Base** | 400,000 $ReGen ($4,000) |
| **Max w/ bonus** | 520,000 $ReGen ($5,200) |
| **Est. hours/week** | 8 |
| **Assignment** | Open |
| **Active seasons** | Spring, Summer |
| **Card image** | `/images/roles/outreach-writer-card.webp` |
| **Scene image** | `/images/roles/outreach-writer-scene.webp` |

**Deliverables:**
- Write 2+ outreach sequences per season
- Maintain email templates and adapt for each campaign
- Track open rates and conversion metrics
- Collaborate with Alliance Weaver on investor messaging

**Seed:** Sequences written and sent on schedule (target: 2+).
**Harvest:** People who responded or applied (actual human engagement, not open rates).

**Powers:**
- Draft outreach sequences for community review
- A/B test subject lines and messaging
- Recommend audience segmentation
- Commission testimonials from land projects

**Rights:**
- Access to outreach skills and email tools
- Authority to send approved campaigns
- Input on audience targeting and messaging strategy

**Domains:** Email campaigns, outreach sequences, audience messaging, campaign metrics

**Character description:** Energetic person with bronze skin and bright expression. Releasing luminous origami light-form birds from raised hands. Each carries a sealed message with colored bioluminescent ribbon. Messenger's crossbody satchel, short solar-membrane-lined cape. Crystalline quill behind ear.

**Scene description:** Balcony on tall living-crystal tower overlooking orchards, terraced gardens, food forests. Jasmine and passionfruit vines with bees. Swallows flying alongside light-birds. Pomegranate tree on balcony rail. Morning light breaking through clouds.

---

### 13. Skills Builder

| Field | Value |
|---|---|
| **Character name** | The Alchemist |
| **Tagline** | Turns code into community tools |
| **Circle** | Tech Circle |
| **Band** | 5 |
| **Base** | 700,000 $ReGen ($7,000) |
| **Max w/ bonus** | 910,000 $ReGen ($9,100) |
| **Est. hours/week** | 10 |
| **Assignment** | Open |
| **Active seasons** | Winter |
| **Card image** | `/images/roles/skills-builder-card.webp` |
| **Scene image** | `/images/roles/skills-builder-scene.webp` |

**Deliverables:**
- Build 2+ new skills per season
- Maintain and improve existing skills based on usage feedback
- Document skill usage patterns and best practices
- Test skills across different Claude interfaces

**Seed:** New skills shipped to the repo (target: 2+).
**Harvest:** Other contributors actively using those skills (adoption tracking).

**Powers:**
- Create new skills and submit to the repo
- Modify existing skills based on contributor feedback
- Set skill documentation standards
- Recommend skill adoption for specific workflows

**Rights:**
- Access to the skill-creator skill and testing framework
- Authority to set skill quality standards
- Input on which skills get prioritized

**Domains:** Skill creation, skill testing, skill documentation, contributor tooling

**Special content:** Build Tools, Get Rewarded section. Cowork referral link: https://claude.ai/referral/v8oHxjZJxg?s=cowork&v=apps

**Character description:** Person with olive skin, bright curious eyes, wild dark hair with bioluminescent streaks. Workbench that's part alchemist's lab, part living organism. Assembling glowing skill-modules. Floating instruction manuals orbit like a halo. Living-fiber lab coat with shifting alchemical/circuit symbols.

**Scene description:** Alchemist's lab inside hollow crystal-tree. Bioluminescent vines bearing glowing berries. Skill-crystals and jars of dried herbs. Curious raven on cauldron rim. Chanterelles and lion's mane growing from walls. Frogs on lily pads. Rainbows refracting through crystal walls.

---

## Season 1 Summary

| Metric | Value |
|---|---|
| **Total roles** | 13 |
| **Roles filled** | 4 (partially) |
| **Roles open** | 9 |
| **Total base budget** | 7,400,000 $ReGen ($74,000) |
| **Total max budget** | 9,620,000 $ReGen ($96,200) |
| **Circles represented** | 8 (Incubation, Alliance, Projects, Community, Anchor, Finance, Communications, Tech) |
| **Art style** | Solarpunk / regenerative future |
| **Image count** | 26 (13 card portraits + 13 full scenes) |
| **Band range** | 300,000 to 900,000 $ReGen (3:1 ratio) |

## Scorecard (fill at Season Festival)

_Each role holder rates 1-5. Blank means the role was unfilled._

| Role | Holder | Scope (1-5) | Comp (1-5) | Impact (1-5) | What would you change? |
|---|---|---|---|---|---|
| Season Facilitator | | | | | |
| Alliance Weaver | | | | | |
| Incubator Guide | | | | | |
| Forum Gardener | | | | | |
| Game Designer | | | | | |
| Treasury Steward | | | | | |
| Storyteller | | | | | |
| Grand Builder | | | | | |
| Security Reviewer | | | | | |
| Tool Curator | | | | | |
| Quest Steward | | | | | |
| Outreach Writer | | | | | |
| Skills Builder | | | | | |

## Lessons (fill at Season Festival)

**What worked:**

**What to change for Season 2:**

**Roles to add/remove/merge:**

**Compensation adjustments:**
