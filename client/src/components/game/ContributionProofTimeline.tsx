  ...
  if (!isLoading && entries.length === 0) {
    return (
      <div className={`text-center py-12 ${className}`}>
        <Leaf className="w-8 h-8 mx-auto mb-3 text-white/70" />
    ...
    <div className={className}>
      ...
      {hasMore && (
        <button
          ...
          className="mt-4 flex items-center gap-1.5 text-xs text-white/70 hover:text-white/80 transition-colors mx-auto"
        >
          <ChevronDown
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
              ...
              {entry.points != null && entry.points > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/5 text-white/70 border border-white/10">
                  +{entry.points} pts
                </span>
              )}
              {isPrivate && (
    ...
    case "text":
      return artifact.excerpt ? (
        <div className="mt-2 flex items-start gap-1.5">
          <FileText className="w-3.5 h-3.5 text-white/70 mt-0.5 flex-shrink-0" />
