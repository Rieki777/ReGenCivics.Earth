...
function MiniSpark({ values, color }: { values: number[]; color: string }) {
  ...
  if (values.length < 2 || values.every((value) => value === 0)) {
    return (
      <svg width={72} height={32} viewBox="0 0 72 32" aria-hidden>
        <line
          x1="4"
          y1="22"
          x2="68"
...
function VolumeTooltip({
  ...
  const rows = payload.filter((entry) => entry.name === "All events" || Number(entry.value) > 0);
  ...
  return (
    <article className={`${CARD} p-4`}>
      <div className="flex items-center gap-2 min-w-0">
        ...
        <h3 className="text-[11px] font-semibold uppercase tracking-wide text-[#1a472a]/80">
          {label}
        </h3>
      </div>
    ...
    <section className={`${CARD} p-4 md:p-5`} aria-label="Apply conversion">
      ...
      <ol className="flex items-stretch gap-2 overflow-x-auto pb-1">
        {steps.map((step, index) => (
          <li key={step.event} className="flex items-center gap-2 flex-shrink-0">
            {index > 0 && (
