...
export function ComposeBox({ onComposed }: { onComposed: (publicationId: number) => void }) {
  ...
  return (
    <div className="rounded-2xl border-2 border-[#4a7c59]/40 bg-white p-4 space-y-2">
      <p className="text-sm font-semibold text-[#1a472a] flex items-center gap-1.5"><PenLine className="w-4 h-4 text-[#4a7c59]" /> Compose</p>
      <Textarea value={text} onChange={(e) => { setText(e.target.value); setPreview(null); }}
        ...
        className="min-h-[90px] text-sm rounded-xl border-[#4a7c59]/30 focus-visible:ring-[#7dd87d]" />
      <div className="flex items-center gap-2 flex-wrap">
        <Button size="sm" variant="outline" className="h-9 rounded-lg border-[#4a7c59]/40 text-[#1a472a]"
      ...
      {preview && (
        <div className="text-xs text-[#1a472a]/70 border-t border-[#1a472a]/10 pt-2">
...
function TargetRow({ publicationId, target, item, onChanged }: {
  ...
  const statusColor = target.status === "published" ? "bg-[#4a7c59] text-white"
    ...
    : "bg-[#1a472a]/10 text-[#1a472a]/70";

  return (
    <div className="rounded-xl border border-[#4a7c59]/20 px-3 py-2 space-y-1.5">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {target.externalUrl && !target.externalUrl.startsWith("buffer:") && (
          <a href={target.externalUrl} target="_blank" rel="noreferrer" className="text-xs text-[#4a7c59] hover:underline inline-flex items-center gap-1">
            <ExternalLink className="w-3 h-3" /> view
          </a>
        )}
        <span className="flex-1" />
        {target.status === "draft" && (
          <Button size="sm" variant="outline" className="h-7 rounded-lg text-xs border-[#4a7c59]/40 text-[#1a472a]" disabled={approve.isPending || !item?.body}
      ...
      {item?.body && <p className="text-xs text-[#1a472a]/60 line-clamp-2">{item.body.slice(0, 200)}</p>}
      {target.surface === "email" && target.status !== "published" && (
        <p className="text-[11px] text-[#1a472a]/50">Email only goes out through the hardened send: edit the newsletter draft below, then Preview email send.</p>
      )}
      {target.surface === "site" && target.status === "approved" && (
        <p className="text-[11px] text-[#1a472a]/50">First publish creates a hidden preview at a private URL; publishing again makes it public (voice grader must pass).</p>
      )}
      {needsProfile && target.status === "approved" && (
        <input value={profileId} onChange={(e) => setProfileId(e.target.value)} placeholder="Buffer profile id for this channel"
          className="w-full text-xs rounded-lg border border-[#4a7c59]/30 px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#7dd87d]" maxLength={64} />
      )}
      {note && <p className="text-[11px] text-[#4a7c59]">{note}</p>}
...
export function PublicationReview({ publicationId }: { publicationId: number }) {
  ...
  if (review.isLoading) return <p className="text-xs text-[#1a472a]/50 py-2"><Loader2 className="w-3 h-3 animate-spin inline mr-1" />Loading publication...</p>;
  if (!review.data) return null;
  const { publication, targets, items, images, article } = review.data;

  return (
    <div className="rounded-2xl border-2 border-[#1a472a]/25 bg-[#fbfaf7] p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <p className="text-sm font-semibold text-[#1a472a]">Publication: {publication.title}</p>
        <Badge variant="outline" className="text-[10px] border-[#4a7c59]/40 text-[#4a7c59]">{publication.status}</Badge>
      ...
      <p className="text-xs text-[#1a472a]/60">Everything below goes out only after you approve it, surface by surface. Edit any draft in the Drafts tier first; the texts here are those same items.</p>
      ...
      <div className="border-t border-[#1a472a]/10 pt-2 space-y-2">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="text-xs font-semibold text-[#1a472a]/70 uppercase tracking-wide">Images</p>
          {(["hero", "inline"] as const).map((slot) => (
            <Button key={slot} size="sm" variant="outline" className="h-7 rounded-lg text-xs border-[#4a7c59]/40 text-[#1a472a]"
        ...
        {images.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {images.map((image) => (
              <button key={image.id} className={`relative rounded-lg overflow-hidden border-2 ${image.chosen ? "border-[#4a7c59]" : "border-transparent hover:border-[#4a7c59]/40"}`}
