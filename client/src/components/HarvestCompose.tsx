...
export function ComposeBox({ onComposed }: { onComposed: (publicationId: number) => void }) {
  ...
  return (
        <div className="rounded-2xl border-2 border-[#1a472a]/30 bg-white p-4 space-y-2">
              <p className="text-sm font-semibold text-[#1a472a] flex items-center gap-1.5"><PenLine className="w-4 h-4 text-[#2d5a3d]" /> Compose</p>p>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {compose.isError && <p className="text-xs text-red-700">{compose.error.message}</p>p>}
              </div>div>
          {preview && (
                  <div className="text-xs text-[#1a472a] border-t border-[#1a472a]/10 pt-2">
                    {preview.ideas.length > 0 ? (
                                <>
                                              <p className="font-semibold mb-1">Drawing from {preview.ideas.length} related idea{preview.ideas.length === 1 ? "" : "s"} ({preview.sourceRefs.length} raw sources):</p>p>
                                  {preview.ideas.map((i) => <p key={i.id}> {i.title}</p>p>)}
                                </>>
                              ) : (
                                <p>No related vault material found; it will draft from your composed words alone.</p>p>
                            )}
                  </div>div>
              )}
        </div>div>
...
function TargetRow({ publicationId, target, item, onChanged }: {
    ...
    return (
          <div className="rounded-xl border border-[#1a472a]/20 px-3 py-2 space-y-1.5">
                <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-medium text-[#1a472a]">{SURFACE_LABEL[target.surface] ?? target.surface}</span>span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full ${statusColor}`}>{target.status}</span>span>
                  {item?.body && (
            ...
            {item?.body && open && (
                    <div className="space-y-2">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                          <span className="text-[11px] text-[#2d5a3d]">Saving teaches the voice loop. Was the edit:</span>span>
                                {(["style", "content"] as const).map((kind) => (
                                    <Button
                                      ...
                                      {editItem.isPending ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : null}
                                          ...
                                          <span className="flex-1" />
                                {saved && <span className="text-[11px] text-[#1a472a]">Saved</span>span>}
                    ...
                    <div className={`rounded-lg border px-2 py-1.5 space-y-1 ${blocks.length > 0 ? "border-red-200 bg-red-50" : "border-amber-200 bg-amber-50"}`}>
                      {flags.map((f, i) => (
                                  <p key={i} className="text-[11px] leading-snug">
                                                <span className={`mr-1.5 px-1 rounded text-[10px] ${f.severity === "block" ? "bg-red-200 text-red-900" : "bg-amber-200 text-amber-900"}`}>{f.severity}</span>span>
                                                <span className="italic text-[#1a472a]">&ldquo;{f.claim}&rdquo;</span>span>
                                                <span className="text-[#2d5a3d]"> {f.problem}</span>span>
                                  </p>p>
                                ))}
                    </div>div>
                )}
            ...
            {takesFirstComment && target.status !== "published" && (
                    <div className="space-y-1">
                              <div className="flex items-center gap-2">
                                          <span className="text-[11px] font-semibold text-[#2d5a3d] uppercase tracking-wide">First comment</span>span>
                                          <span className="text-[10px] text-[#2d5a3d]/80">the link goes here, not in the post</span>span>
                                          <span className="flex-1" />
            ...
            {target.status === "published" && (
                    <div className="space-y-1">
                              <span className="text-[11px] font-semibold text-[#2d5a3d] uppercase tracking-wide">Weekly note</span>span>
                              <Textarea
            ...
            {target.surface === "email" && target.status !== "published" && item?.body && (
                    <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className="text-[11px] text-[#2d5a3d]">Opens Outbound Write with this draft filled. Send stays on Preview there.</span>span>
                    </div>div>
                )}
            ...
            {target.surface === "email" && target.status !== "published" && item && item.status !== "edited" && (
                    <p className="text-[11px] text-[#2d5a3d]">Email goes out through the hardened send on this draft. Save an edit first, even a small one, then preview and confirm here. Or Continue as newsletter to finish in Outbound Write.</p>p>
                )}
            {target.surface === "site" && target.status === "approved" && (
                    <p className="text-[11px] text-[#2d5a3d]">First publish creates a hidden preview at a private URL; publishing again makes it public (voice grader must pass).</p>p>
                )}
            {note && <p className="text-[11px] text-[#2d5a3d]">{note}</p>p>}
            {publish.isError && <p className="text-[11px] text-red-700">{publish.error.message}</p>p>}
          </div>div>
        );
}
...
export function PublicationReview({ publicationId }: { publicationId: number }) {
    ...
    if (review.isLoading) return <p className="text-xs text-[#2d5a3d] py-2"><Loader2 className="w-3 h-3 animate-spin inline mr-1" />Loading publication...</p>p>;
    ...
    return (
          <div className="rounded-2xl border-2 border-[#1a472a]/25 bg-[#f8f5f0] p-4 space-y-3">
                <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-[#1a472a]">Publication: {publication.title}</p>p>
                        <Badge variant="outline" className="text-[10px] border-[#1a472a]/30 text-[#2d5a3d]">{publication.status}</Badge>Badge>
                ...
                <p className="text-xs text-[#2d5a3d]">Everything below goes out only after you approve it, surface by surface. Edit the text here. After you save an email draft, preview and confirm send on that row, or Continue as newsletter into Outbound Write. Social posts open in Outbound Social.</p>p>
                ...
                <div className="border-t border-[#1a472a]/10 pt-2 space-y-2">
                        <div className="flex items-center gap-2 flex-wrap">
                                  <p className="text-xs font-semibold text-[#1a472a] uppercase tracking-wide">Images</p>p>
                        ...
                        <Button key={slot} size="sm" variant="outline" className="h-7 rounded-lg text-xs border-[#1a472a]/30 text-[#1a472a]"
                          ...
                          {generateImages.isError && <p className="text-[11px] text-red-700">{generateImages.error.message}</p>p>}
                        </div>div>
                  {images.length > 0 && (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {images.map((image) => (
                                      <button key={image.id} className={`relative rounded-lg overflow-hidden border-2 ${image.chosen ? "border-[#4a7c59]" : "border-transparent hover:border-[#1a472a]/30"}`}
                                                      ...
                                                      <span className="absolute top-1 left-1 text-[9px] px-1.5 py-0.5 rounded bg-black/50 text-white">{image.slot}{image.chosen ? " · chosen" : ""}</span>span>
                                      </button>button>
                                    ))}
                      </div>div>
                        )}
