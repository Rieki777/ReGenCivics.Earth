...
import {
  ...
  groupGeneralInquiryAnswers,
  type FormattedInquiryAnswer,
} from "@shared/generalInquiryFields";

function AnswerValue({ answer }: { answer: FormattedInquiryAnswer }) {
  if (answer.isEmpty) {
...
/** Full general/alliance inquiry answers — every registry field (empty → "—"). */
export function GeneralInquiryAnswersPanel({
  inquiry,
  ...
  const groups = groupGeneralInquiryAnswers(
    formatGeneralInquiryAnswers(inquiry as Record<string, unknown> | null | undefined),
  );
