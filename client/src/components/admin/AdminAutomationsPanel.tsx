...
export function AdminAutomationsPanel() {
  ...
  return (
    <div className="rounded-2xl border border-[#1a472a]/12 bg-white p-5">
      ...
      <p className="text-[#1a472a]/60 text-xs mb-3">Updates your assistants prepare for you on a schedule. Read-only, never auto-changes data.</p>

      {rows.length === 0 ? (
        <p className="text-[#1a472a]/50 text-sm py-4 text-center">No routines yet. Add one above and it runs on its cadence.</p>
      ) : (
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`font-semibold text-sm ${a.enabled ? "text-[#1a472a]" : "text-[#1a472a]/40"}`}>{a.name}</span>
                    <span className="text-[10px] uppercase tracking-wide text-[#1a472a]/50 bg-[#1a472a]/5 rounded-full px-2 py-0.5">{TYPE_LABEL[a.type] ?? a.type}</span>
                    <span className="inline-flex items-center gap-1 text-[11px] text-[#1a472a]/50"><Clock className="w-3 h-3" />{a.cadence}</span>
                  </div>
                  {a.lastResult && <p className="text-[#1a472a]/70 text-xs mt-1 break-words">{a.lastResult}</p>}
                  {a.lastRunAt && <p className="text-[#1a472a]/40 text-[11px] mt-0.5">Last run {new Date(a.lastRunAt as unknown as string).toLocaleString()}</p>}
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <button type="button" title={a.enabled ? "Disable" : "Enable"} onClick={() => toggle.mutate({ id: a.id, enabled: !a.enabled })} className={`w-9 h-9 rounded-full flex items-center justify-center ${a.enabled ? "text-[#7dd87d] hover:bg-[#7dd87d]/15" : "text-[#1a472a]/40 hover:bg-[#1a472a]/5"}`}>
