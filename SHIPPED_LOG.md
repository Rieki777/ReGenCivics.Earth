...
- **SHIP_V5_FLYWHEEL (partial):** State of the Ship public trust dashboard (`ship.stateOfShip` tiles on `/ship`) and the orientation gate (a booking can't go `active` until the Keeper marks orientation; logged override). **Deferred:** gear manifest (§1), Homecoming recap pages (§2), crew list capture (§4) — schema already provisioned in migration 0183 (`ship_gear_checks`, `ship_crew_list_signups`).
