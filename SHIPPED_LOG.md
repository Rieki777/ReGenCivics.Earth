...
## 2026-07-10 (latest): Ship Quest v2 (points threshold, weighted draws, nominations, crew sponsorship)
...
- Migration `drizzle/0179_ship_quest_v2.sql` applied to production. Tests: threshold 149/150, weighted-draw determinism + audit + nominee entry + prior-winner exclusion, sponsorship accumulation + goal flip, router guards (crew.sponsor precondition, crew auth), copy greps. Full suite **341 green**. Source spec: `CLAUDE_CODE_PROMPT_2026-07-10_SHIP_QUEST_V2.md`. Defaults (X=150, points-as-tickets, nominees at 150 tickets, first-to-150 maiden voyage, $2,100 goal) shipped as specified; Rye to bless or veto and add the drawing-weighting + nomination + sponsorship changes to the counsel review packet.
