...
- Verify: typecheck exit 0, truncation 0/0, vite build green (workbox PNG failure is the known local-only issue), 100 tests pass across companion/guide/applications/elder/ship/ship-map (3 new Gardener tests: registry + enum constraints + field-invention defense). Live prod flags: `companion: true`, `serverStt: false`, `hostedTts: false` (STT/TTS keys still Rye carryover).
